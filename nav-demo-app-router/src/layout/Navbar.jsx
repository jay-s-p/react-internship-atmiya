      ...
      <nav className="bg-gray-100  fixed w-full h-fit z-50 md:h-fit">
        <div className="mx-auto h-fit">
          <div className="h-fit flex flex-col items-center justify-between 
                          md:flex-row ">
            <input className='peer hidden navbar-main' type="checkbox" id="navbar-inp" />
            {/* Title text */}
            ...
            {/* Navigation Links */}
            <div className="hidden peer-checked:block w-full h-[95vh] ring-0 ring-inset ring-red-500 bg-gray-200  mx-5
             ...
             md:text-xl md:flex-row md:justify-end">
                {
                  navItems.map((nav, id) => <NavItems className={isActive(nav[1])} navText={nav[0]} navPath={nav[1]} name="nav-bar" />)
                }
              </div>
            </div>
