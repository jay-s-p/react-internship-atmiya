...
const SMSCreditBalance = ({ sms_credit_balance }) => {
  return (
    <div style={{ background: "#141e4a", opacity: "0", animation: "element-scale 500ms ease 0ms 1 forwards" }} className=' rounded-4 text-white px-3 pt-4 pb-3'>
      <TextImage text={
        <div className='d-flex flex-column m-0'>
          ...
          <h5>{sms_credit_balance?.toLocaleString('en-IN')}</h5>
        </div>
      } image={
...
const WhatsAppCreditBalance = ({ whatsapp_credit_breakup }) => {
  return (
    <div style={{ background: "#141e4a", opacity: "0", animation: "element-scale 500ms ease 100ms 1 forwards" }} className=' rounded-4 text-white px-3 pt-4 pb-3 mt-3'>
      <TextImage text={
        <div className='d-flex flex-column m-0'>
          ...
          <h5>{whatsapp_credit_breakup?.toLocaleString('en-IN')}</h5>
        </div>
      } image={
        <div className='ms-auto'>
          <svg xmlns="http://www.w3.org/2000/svg" width={50} height={50} fill="white" className="bi bi-telegram" viewBox="0 0 16 16">
...
const PaymentBreakup = ({payment_breakup}) => {
  return (
    <div className='mt-3'>
      <DashboardCard style={{ opacity: "0", animation: "element-scale 500ms ease 200ms 1 forwards" }} title="💳 Payment Breakup">
        <LineText >
          ...
          <div className='fs-5 ms-auto my-auto'>{payment_breakup.amazon?.toLocaleString('en-IN')}</div>
        </LineText>
        <LineText color="#f18282">
          <div>
            <div>Card</div>
          </div>
          <div className='fs-5 ms-auto my-auto'>{payment_breakup.card?.toLocaleString('en-IN')}</div>
        </LineText>
        <LineText color="#c581e8">
          <div>
            <div>Cash</div>
          </div>
          <div className='fs-5 ms-auto my-auto'>{payment_breakup.cash?.toLocaleString('en-IN')}</div>
        </LineText>
      </DashboardCard>
    </div>
  )
...
const DashboardMainRight = ({ data }) => {
  return (
    <>
      <div className="col-12 col-xl-3 m-0 p-0 overflow-hidden mt-3">
        <div className='px-3 m-0 w-100 mt-3'>
          <SMSCreditBalance sms_credit_balance={data.sms_credit_balance} />
          <WhatsAppCreditBalance whatsapp_credit_breakup={data.whatsapp_credit_breakup} />
          <PaymentBreakup payment_breakup={data.payment_breakup} />
        </div>
      </div>
    </>
  )
